# Real Single Static Range Dataset 实数单目标静止距离数据集
import os
...
from apps.irpoc.fmcw_config import FmcwConfig as Fcfg

class RssrDs(Dataset):
    def __init__(self, num:int = 500, mode:str = 'train'):
        # self.convert_ds()
        self.X = torch.load(f'./work/datasets/fmcw/fmcw_ssr_X_{mode}.pt')
        self.y = torch.load(f'./work/datasets/fmcw/fmcw_ssr_y_{mode}.pt')

    def convert_ds(self) -> None:
        X = []
        with open('./work/datasets/fmcw/irpoc_fmcw_X.txt', 'r', encoding='utf-8') as xfd:
            for row in xfd:
                row = row.strip()
                arrs = row.split(',')
        ...
        torch.save(self.X, f'./work/datasets/fmcw/fmcw_ssr_X_train.pt')
        print(f'### self.X: {self.X.shape}; {self.X.dtype};')
        y = []
        with open('./work/datasets/fmcw/irpoc_fmcw_y.txt', 'r', encoding='utf-8') as yfd:
            for row in yfd:
                row = row.strip()
                yi = np.array([complex(float(row), 0)], dtype=np.complex64)
                y.append(yi)
        self.y = np.array(y, dtype=np.complex64)
        torch.save(self.y, f'./work/datasets/fmcw/fmcw_ssr_y_train.pt')
        ...
        torch.save(X_t, f'./work/datasets/fmcw/fmcw_ssr_X_test.pt')
        y_t = np.array(y_t_, dtype=np.complex64)
        torch.save(y_t, f'./work/datasets/fmcw/fmcw_ssr_y_test.pt')
        X_v = np.array(X_v_, dtype=np.complex64)
        torch.save(X_v, f'./work/datasets/fmcw/fmcw_ssr_X_val.pt')
        y_v = np.array(y_v_, dtype=np.complex64)
        torch.save(y_v, f'./work/datasets/fmcw/fmcw_ssr_y_val.pt')
